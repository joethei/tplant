  ...
  export function create(signature: ts.Symbol, namedDeclaration: ts.NamedDeclaration, checker: ts.TypeChecker): Method {
    ...
    if (methodSignature !== undefined) {
      ...
      if (methodSignature.typeParameters !== undefined) {
        for (const typeParameter of methodSignature.typeParameters) {
          if (typeParameter.symbol.declarations) {
            const created = TypeParameterFactory.create(typeParameter.symbol, typeParameter.symbol.declarations[0], checker);
            result.typeParameters.push(created);
          }
        }
      }
    }
